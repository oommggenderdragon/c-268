...
def upload_image():
    ...
    if operation_selection == 'gray':
        ...
        file_data = rbg_effect(decode_array_to_img)
...
def water_color_effect(decode_array_to_img):
    water_color_effect = cv2.styization(decode_array_to_img,sigma_s=60,sigma_r = 0.6)
    status,output_img = cv2.imencode('.PNG',water_effect)
    return output_img

def invert(decode_array_to_img):
    invert_effect = cv2.cv2.bitwise_not(decode_array_to_img)
...
def HDR(decode_array_to_img):
    hdr_effect = cv2.detailEnchance(decode_array_to_img,sigma_s=12,sigma_r= 0.15)
